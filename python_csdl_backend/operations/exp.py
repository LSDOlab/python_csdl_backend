...
class ExpLite(OperationBase):
    ...
    def get_evaluation(self, eval_block, vars):
        ...
        eval_block.write(f'{self.output_name} = np.exp({self.input_name})')
    ...
    def get_partials(self, partials_dict, partials_block, vars, is_sparse_jac):
        ...
        # OLD FULL JACOBIAN
        # if is_sparse_jac:
        #     partials_block.write(f'{partial_name} = sp.diags(np.exp({input}).flatten(), format = \'csc\')')
        # else:
        #     partials_block.write(f'{partial_name} = np.diag(np.exp({input}).flatten())')

        # NEW:
        # only return diag values for elementwise
        # Also sparsity doesn't matter
        partials_block.write(f'{partial_name} = np.exp({input}).flatten()')
