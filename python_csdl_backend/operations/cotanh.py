...
def get_cotanh_lite(op):
    return CotanhLite


class CotanhLite(OperationBase):
    ...
    def get_evaluation(self, eval_block, vars):
        ...
        eval_block.write(f'{self.output_name} = 1.0/np.tanh({self.input_name})')
    ...
    def get_partials(self, partials_dict, partials_block, vars, is_sparse_jac):
        ...
        # OLD FULL JACOBIAN
        inner_str = f'(1.0 - (1.0 / np.tanh({input}))**2).flatten()'
        # if is_sparse_jac:
