...
def get_cosec_lite(op):
    return CosecLite


class CosecLite(OperationBase):
    ...
    def get_evaluation(self, eval_block, vars):
        ...
        eval_block.write(f'{self.output_name} = 1.0/np.sin({self.input_name})')
    ...
    def get_partials(self, partials_dict, partials_block, vars, is_sparse_jac):
        ...
        # OLD FULL JACOBIAN
        inner_str = f'-1.0*(np.cos({input})/(np.sin({input}) * np.sin({input}))).flatten()'
        # if is_sparse_jac:
