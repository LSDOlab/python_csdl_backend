...
def get_sech_lite(op):
    return SechLite


class SechLite(OperationBase):
    ...
    def get_evaluation(self, eval_block, vars):
        ...
        eval_block.write(f'{self.output_name} = 1.0/np.cosh({self.input_name})')
    ...
    def get_partials(self, partials_dict, partials_block, vars, is_sparse_jac):
        ...
        inner_str = f'(-1.0 * np.tanh( {input}) * (1.0 / np.cosh({input}))).flatten()'
        if is_sparse_jac:
