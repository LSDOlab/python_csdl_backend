...
from python_csdl_backend.utils.general_utils import get_deriv_name, to_unique_list, lineup_string, set_opt_upper_lower, set_scaler_array
...
class Simulator(SimulatorBase):
    ...
    def __init__(self,
        ...
        if comm:
            ...
            from python_csdl_backend.dag_analyzer.compiler.generator import code_generator
            from python_csdl_backend.dag_analyzer.compiler.run_code import run_code 
            import time
            ...
            schedule, raw_variable_owner_map = scheduler.schedule(
                ccl_graph,
                profile = PROFILE,
            ...
            for node in raw_variable_owner_map:
                ...
                if node in str2nodes:
                    self.system_graph.variable_owner_map_full[str2nodes[node]] = raw_variable_owner_map[node]
                    self.system_graph.variable_owner_map[str2nodes[node]] = list(raw_variable_owner_map[node])[0]
                else:
        ...
        for of_name in ofs:
            for wrt_name in wrts:
                ...
                if self.comm is not None:
                    ...
                    owner_rank = self.system_graph.variable_owner_map[wrt_node]
    ...
    def __getitem__(self, key):
        ...
        if unique_id:
            if self.comm is not None:
                owner_rank = self.system_graph.variable_owner_map[self.system_graph.unique_to_node[unique_id]]
    ...
    def _compute_fd_partial(self, input_name, input_dict, outputs_dict, delta):
        ...
        for col_index in range(input_size):
            ...
            for output_name in outputs_dict:
                ...
                if self.comm is not None:
                    owner_rank = self.system_graph.variable_owner_map[self.system_graph.unique_to_node[output_id]]
