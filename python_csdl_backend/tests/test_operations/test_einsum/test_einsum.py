...
class ExampleInnerVectorVector(Model):
    ...
    def define(self):
        ...
        vec = self.create_input('a', val=a)

        # Inner Product of 2 vectors
        self.register_output('einsum_inner1',
                             csdl.einsum(1*vec, vec, subscripts='i,i->'))


def test_expand_ivv():
    a = np.arange(4)
    vals_dict = {'einsum_inner1': np.einsum('i,i->', a, a)}
    totals_dict = {}

    run_test(
        ExampleInnerVectorVector(),
        outs=['einsum_inner1'],
        ins=['a'],
        name='test_expand_ivv',
        vals_dict=vals_dict,
...
class ExampleInnerTensorVector(Model):
    ...
    def define(self):
        ...
        # Declaring tensor
        tens = self.create_input('c', val=c)
...
def test_expand_itv():
    ...
    vals_dict = {'einsum_inner2': np.einsum('ijk,j->ik', c, a)}
    totals_dict = {}

    run_test(
        ExampleInnerTensorVector(),
        outs=['einsum_inner2'],
        ins=['a'],
        name='test_expand_itv',
        vals_dict=vals_dict,
        totals_dict=totals_dict)
